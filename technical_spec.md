...
1. **Sale.** (Unchanged from current law.) Sale includes any disposition of appreciated property in exchange for *other property* — a barter sale is a sale. Buying a house with appreciated private stock, or acquiring a company in an all-stock deal, realizes the gain on the property given up, valued at the transaction's agreed price. This is already the rule under existing law (§1001); the only deferral carve-out was §1031 like-kind exchanges, which this framework eliminates (see Interactions table). No new machinery is required, and no appraisal is needed: the counterparty's acceptance sets the price, exactly as the borrowing LTV floor does.

   **Carve — pure stock-for-stock reorganizations (§368 continuity-of-interest).** One narrow exchange is *not* an exit: a passive shareholder whose shares are swapped for acquirer shares in an all-stock merger receives no cash and no asset they can independently consume or control. Their illiquid holding is merely transformed. Forcing realization here creates a tax bill with no means to pay it and would freeze ordinary M&A. Treatment: **carryover basis and carryover lifetime counter into the new shares, no step-up.** This is deferral, not forgiveness — the embedded gain rides into the new paper and is taxed at the real exit (sale, borrowing, gift, or death). The operative line is the framework's own test: realization occurs when an exchange yields cash, a consumable/controllable asset, or cashes out a counterparty; a pure reorganization does none of these. Any cash or non-stock consideration received ("boot") is realized dollar-for-dollar.

   *Why this carve doesn't reopen buy-borrow-die:* a holder could roll appreciated stock through serial reorganizations and never sell — but the moment they borrow against the new shares, Rule 3's borrowing trigger fires. The reorganization carve is safe *only because* the borrowing rule already closes that exit. Absent Rule 3's borrowing leg, this carve would be a hole; with it, it is not.
...
#### Section 121 Treatment (Primary Residence)
...
### Worked Example: Successful Founder, Single Filer
...
- rate at $2M = 0.37 × (2M − 2M) / 4M = 0%
- rate at $6M = 0.37 × (6M − 2M) / 4M = 37%
- tax_band = ((0% + 37%) / 2) × $4,000,000 = 18.5% × $4,000,000 = **$740,000**

*Above C ($6M → $19.5M = $13.5M of gain):* taxed at 37%.
- tax_above = 37% × $13,500,000 = **$4,995,000**
...
- $15M QSBS exclusion → $0 tax on the first $15M
- Remaining $4M at 23.8% (LTCG + NIIT) = $952,000
- Total current-law tax: ~$952,000
...
5. **State interaction.** The framework is federal. State capital gains taxes (which vary widely) are unaffected. State estate taxes may need separate treatment. (Also addressed in Section 0.)
6. **MFJ counter architecture.** Two-counter (current spec) vs. single-combined approaches each have edge cases at marriage, divorce, and death. See discussion in Section 3.

*Resolved in v0.3:*
- ~~Charitable contributions of appreciated assets.~~ Resolved as a Rule 3 exception with basis-only deduction. See Rule 3 → Charitable Donation Treatment.
