# Infographic Prompt for AI Image Generator

> **NEEDS REGENERATION.** The published image (and the spec below) uses old $2.5M / $10M thresholds. Canonical per the published Lifetime Gains essay: $2M (exemption) / $6M (ceiling), both CPI-indexed. The crossover point shifts from ~$6.3M to roughly $4.6M. Revenue claim should read $45-170B (midpoint ~$108B), not $85-200B. Update all figures during regeneration.

Create a clean, minimal infographic on a white background. No clip art, no icons, no flags, no illustrations. This should look like a chart from The Economist or a policy brief, not a campaign ad.

**Title** (top left, bold, dark charcoal): "The Lifetime Gains Framework"
**Subtitle** (below title, lighter gray): "How the tax rate changes with cumulative lifetime capital gains"

**The chart** (takes up 70% of the image):

A line chart with clearly labeled axes.

X-axis label: "Cumulative Lifetime Capital Gains"
X-axis tick marks at: $0, $2.5M, $5M, $7.5M, $10M, $15M, $20M

Y-axis label: "Marginal Tax Rate"
Y-axis tick marks at: 0%, 10%, 20%, 30%, 37%

**Line 1 (the framework)** — bold, dark blue or teal:
- Flat at 0% from $0 to $2.5M
- Rises linearly from 0% at $2.5M to 37% at $10M
- Flat at 37% from $10M onward

**Line 2 (current law)** — dashed, gray or light red:
- Flat at 23.8% across the entire chart
- Labeled "Current Law: 23.8% flat rate"

**Shaded zones** (subtle, low opacity):
- $0 to $2.5M: light green fill, labeled "Tax-Free Zone"
- $2.5M to $10M: light amber fill, labeled "Sliding Scale"
- $10M+: light coral/red fill, labeled "Full Rate"

**The intersection point** where the framework line crosses the current law line (approximately $6.3M) should be marked with a small dot and labeled "Crossover: ~$6.3M"

Below the crossover label, a small annotation: "Below this point, the framework charges less than current law. Above it, more."

**Bottom bar** — three compact stats in a horizontal row, small text:

"98% of Americans pay the same or less" | "$85–200B in annual deficit reduction" | "Eliminates 10–15% of the Internal Revenue Code"

**Footer** (very small, gray): "mattsly.com/fair-and-simple-tax-act · Thresholds are proposed starting points, indexed to CPI"

**Style requirements:**
- White or off-white background
- Clean sans-serif font (Inter, Helvetica, DM Sans, or similar)
- No gradients, no drop shadows, no 3D effects, no decorative elements
- Color palette limited to: dark charcoal text, teal/blue for the framework line, gray for current law, green/amber/coral for zone fills
- Aspect ratio: 16:9 landscape for social sharing
- Overall feel: data visualization, not marketing material
